...
class UNetDecoderBlock1(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels, act_fn=nn.ReLU(inplace=True)):
        ...
        self.conv1 = nn.Conv2d(skip_channels*2, skip_channels, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(skip_channels)
        self.act1 = act_fn
    ...
    def forward(self, x, skip):
        ...
        x = self.act2(self.bn2(self.conv2(x)))
        return x
class UNetDecoderBlock1_3(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels, act_fn=nn.ReLU(inplace=True)):
        super(UNetDecoderBlock1_3, self).__init__()
        self.upsample = nn.ConvTranspose2d(in_channels, skip_channels, kernel_size=2, stride=2)
        self.conv1 = nn.Conv2d(skip_channels*2, skip_channels, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(skip_channels)
        self.act1 = act_fn
        self.conv2 = nn.Conv2d(skip_channels, out_channels, kernel_size=3, padding=1)
...
class ModifiedResNet50(nn.Module):
    def __init__(self,ngf):
        ...
        self.conv0_1 = nn.Conv2d(64, 128, kernel_size=1)
        self.conv0_2 = nn.Conv2d(128, 256, kernel_size=1)
        self.tem_branch2 = FusionTemporalBlock(256)
        self.spat_branch2 = EchocardiographySpatialAttentionBlock(256)
        self.tem_branch3 = FusionTemporalBlock(512)
        self.spat_branch3 = EchocardiographySpatialAttentionBlock(512)
        self.tem_branch4 = FusionTemporalBlock(1024)
        self.spat_branch4 = EchocardiographySpatialAttentionBlock(1024)
        self.tem_branch5 = FusionTemporalBlock(2048)
        self.spat_branch5 = EchocardiographySpatialAttentionBlock(2048)

        self.fusion_expertset1 = CrossLayerExpertFusion(256,256,4)
        self.fusion_expertset2 = CrossLayerExpertFusion(256,512,4)
        self.fusion_expertset3 = CrossLayerExpertFusion(512,1024,4)
        self.fusion_expertset4 = CrossLayerExpertFusion(1024,2048,4)
        # # 冻结所有参数（可选）
        ...
        self.deconv_0_1 = UNetDecoderBlock(2048, 1024, 1024)
        self.deconv_1_1 = UNetDecoderBlock(1024, 512, 512)
        self.deconv_2_1 = UNetDecoderBlock(512, 256, 256)
        self.deconv_3_1_ = UNetDecoderBlock1(256, 256, 64)
        self.deconv_31_1_ = UNetDecoderBlock1_3(256, 64, 64)
        self.deconv_4_1 = UNetDecoderBlock(32, 16, 16)
        self.out1 = nn.Sequential(nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2),nn.Tanh(),nn.Conv2d(32, 16, kernel_size=1, stride=1),nn.Tanh(),nn.Conv2d(16, 3, kernel_size=1, stride=1)) #  self.final_out_dim
 ...
 # -----------------------------------layer0---------------------------------------------------------------------------------
        ...
        fusion_layer0 = self.conv0_1(fusion_layer0)
        fusion_layer0 = self.conv0_2(fusion_layer0)
        ...
        x_tem1, confidence_tem1, _, _, _ = self.tem_branch2(ir_feature2_)
        x_spat1, conf_spat1, attn_maps_all_spat1 = self.spat_branch2(ir_feature2_)
        ...
        y_tem2, confidence_tem2, _, _, _ = self.tem_branch2(vis_feature2_)
        y_spat2, conf_spat2, attn_maps_all_spat2 = self.spat_branch2(vis_feature2_)
        ...
        x_tem2, confidence_tem2, _, _, _ = self.tem_branch3(ir_feature3_)
        x_spat2, conf_spat2, attn_maps_all_spat2 = self.spat_branch3(ir_feature3_)
        ...
        y_tem2, confidence_tem2, _, _, _ = self.tem_branch3(vis_feature3_)
        y_spat2, conf_spat2, attn_maps_all_spat2 = self.spat_branch3(vis_feature3_)
        vis_features3__ = y_tem2 + y_spat2 + vis_feature3_  # (bs,16,128,28,28)

        # ir_features3__ = self.dropout1(ir_features3__.view(-1, *ir_features3__.shape[-3:]))
        # ir_features3__ = ir_features3__.view(-1, 16, 128, 32, 32)
        # vis_features3__ = self.dropout1(vis_features3__.view(-1, *vis_features3__.shape[-3:]))
        # vis_features3__ = vis_features3__.view(-1, 16, 128, 32, 32)
 ...
 # -----------------------------------layer3---------------------------------------------------------------------------------
        ...
        x_tem3, confidence_tem3, _, _, _ = self.tem_branch4(ir_feature4_)
        x_spat3, conf_spat3, attn_maps_all_spat3 = self.spat_branch4(ir_feature4_)
        ...
        y_tem3, confidence_tem3, _, _, _ = self.tem_branch4(vis_feature4_)
        y_spat3, conf_spat3, attn_maps_all_spat3 = self.spat_branch4(vis_feature4_)
...
# -----------------------------------layer4---------------------------------------------------------------------------------
        ...
        x_tem, confidence_tem, _, _, _ = self.tem_branch5(ir_feature5_)
        x_spat, conf_spat, attn_maps_all_spat = self.spat_branch5(ir_feature5_)
        ir_features5__ = x_tem + x_spat + ir_feature5_ #(bs,16,512,7,7)
        ...
        y_tem1, confidence_tem1, _, _, _ = self.tem_branch5(vis_feature5_)
        y_spat1, conf_spat1, attn_maps_all_spat1 = self.spat_branch5(vis_feature5_)
        vis_features5__ = y_tem1 + y_spat1 + vis_feature5_  # (bs,16,512,7,7)
...
# -----------------------------------infrared_decoder---------------------------------------------------------------------------------
        ...
        ir_deconv_3_1 = self.deconv_31_1_(ir_deconv_2_1, x_ir1)
...
# -----------------------------------visible decoder---------------------------------------------------------------------------------
        ...
        vis_deconv_3_1 = self.deconv_31_1_(vis_deconv_2_1, y_vis1)
...
if __name__ == "__main__":
   # 初始化模型
    ...
    model = ModifiedResNet50(32).to(device)
    #模拟输入 (bs=2, 16帧)
