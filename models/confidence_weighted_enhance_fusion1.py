...
class MultiScaleFusionBlock(nn.Module):
    def __init__(self, in_channels,out_channels):
        ...
        # 融合调权因子（可调）
        self.ir_weight_amp = 1.0
        self.vis_weight_amp = 1.2
        self.fused_gamma = 1.2
    ...
    def forward(self, ir_feat, vis_feat, prev_fused_feat=None):
        ...
        ir_weighted = ir * edge_mask * self.ir_weight_amp
        vis_weighted = vis_detail * (1 - edge_mask) * self.vis_weight_amp
        ...
        # === 防止融合图偏暗（Gamma增强）===
        # fused = fused.clamp(0, 1)  # 先限制范围
        # fused = fused ** (1 / self.fused_gamma)  # gamma correction
