...
from models.Expert_MoE3 import CrossLayerExpertFusion,CrossLayerExpertFusion_MultiGate,CrossLayerExpertFusion_MultiGate_TokenMoE,CrossLayerExpertFusion_MultiGate_TokenMoE_ExpertSet,CrossLayerExpertFusion_MultiGate_TokenMoE_ExpertSet_no_motion,CrossLayerExpertFusion_MultiGate_TokenMoE_ExpertSet_edgeir
from models.confidence_weighted_enhance_fusion import MultiScaleFusionBlock,MultiScaleFusionBlock1,MultiScaleFusionBlock2
# ========== 多尺度局部注意力辅助函数 ==========
...
class MultiViewInteraction(nn.Module):
    ...
    def forward(self, logits_list):
        ...
        return enhanced
###############################backbone###################################
class ModifiedResNet18_fusion(nn.Module):
    def __init__(self,ngf):
        ...
        self.tem_branch2_D = FusionTemporalBlock(64)
        self.spat_branch2_D = EchocardiographySpatialAttentionBlock(64)
        self.tem_branch3_D = FusionTemporalBlock(128)
        self.spat_branch3_D = EchocardiographySpatialAttentionBlock(128)
        self.tem_branch4_D = FusionTemporalBlock(256)
        self.spat_branch4_D = EchocardiographySpatialAttentionBlock(256)


        self.fusion_weight0 = MultiScaleFusionBlock(64,64)
        self.fusion_weight1 = MultiScaleFusionBlock1(64,64)
        self.fusion_weight2 = MultiScaleFusionBlock2(64,128)
        self.fusion_weight3 = MultiScaleFusionBlock2(128,256)
        self.fusion_weight4 = MultiScaleFusionBlock2(256,512)
        ...
        self.fusion_expertset4 = CrossLayerExpertFusion_MultiGate_TokenMoE_ExpertSet_edgeir(512,512,3)
        # # 冻结所有参数（可选）
 ...
 # -----------------------------------layer0---------------------------------------------------------------------------------
        ...
        fusion_layer0 = self.fusion_weight0(ir_feat=x_ir1,vis_feat=y_vis1, prev_fused_feat=fusion_layer0)
        ...
        fusion_layer1 = self.fusion_weight1( ir_feat=ir_features2__, vis_feat=vis_features2__,prev_fused_feat=fusion_layer0)
        ...
        fusion_layer2 = self.fusion_weight2( ir_feat=ir_features3__, vis_feat=vis_features3__,prev_fused_feat=fusion_layer1)
 ...
 # -----------------------------------layer3---------------------------------------------------------------------------------
        ...
        fusion_layer3 = self.fusion_weight3(ir_feat=ir_features4__, vis_feat=vis_features4__,  prev_fused_feat=fusion_layer2)
...
# -----------------------------------layer4---------------------------------------------------------------------------------
        ...
        fusion_layer4_1 = self.fusion_weight4(ir_feat=ir_features5__, vis_feat=vis_features5__,  prev_fused_feat=fusion_layer3)
        fusion_layer4_ = self.fusion_expertset4(ir_feat=ir_features5__, vis_feat=vis_features5__,  fused_feat=fusion_layer4_1)
        fusion_layer4 = fusion_layer4_["fused_output"]
        lb_loss_4 = fusion_layer4_["load_balancing_loss"]
        sparse_loss_4 = fusion_layer4_["sparsity_loss"]

        # print(fusion_layer4.shape)
        ...
        # deconv_0_1 = self.deconv_0_1(fusion_layer4, fusion_layer3)
        # deconv_0_1 = deconv_0_1.view(-1, 16, *deconv_0_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem3_D, confidence_tem_ir3_D, _, _, _ = self.tem_branch4_D(deconv_0_1)
        # x_spat3_D, conf_spat_ir3_D, attn_maps_all_spat3_D = self.spat_branch4_D(deconv_0_1)
        # deconv_0_1  = x_tem3_D + x_spat3_D +  deconv_0_1 # (bs,16,256,14,14)
        #
        # deconv_0_1 = deconv_0_1.view(-1, *deconv_0_1.shape[-3:])
        # deconv_1_1 = self.deconv_1_1(deconv_0_1, fusion_layer2)
        # deconv_1_1 = deconv_1_1.view(-1, 16, *deconv_1_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem2_D, confidence_tem_ir2_D, _, _, _ = self.tem_branch3_D(deconv_1_1)
        # x_spat2_D, conf_spat_ir2_D, attn_maps_all_spat2_D = self.spat_branch3_D(deconv_1_1)
        # deconv_1_1 = x_tem2_D + x_spat2_D + deconv_1_1  # (bs,16,256,14,14)
        #
        # deconv_1_1 = deconv_1_1.view(-1, *deconv_1_1.shape[-3:])
        # deconv_2_1 = self.deconv_2_1(deconv_1_1, fusion_layer1)
        # deconv_2_1 = deconv_2_1.view(-1, 16, *deconv_2_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem1_D, confidence_tem_ir1_D, _, _, _ = self.tem_branch2_D(deconv_2_1)
        # x_spat1_D, conf_spat_ir1_D, attn_maps_all_spat1_D = self.spat_branch2_D(deconv_2_1)
        # deconv_2_1 = x_tem1_D + x_spat1_D + deconv_2_1  # (bs,16,256,14,14)
        #
        # deconv_2_1 = deconv_2_1.view(-1, *deconv_2_1.shape[-3:])
        # deconv_3_1 = self.deconv_3_1_(deconv_2_1, fusion_layer0)
        # output1 = self.out1(deconv_3_1)
# -----------------------------------infrared_decoder---------------------------------------------------------------------------------
        ...
        # ir_deconv_0_1 = self.deconv_0_1(ir_features5__, ir_features4__)
        # ir_deconv_0_1 =  ir_deconv_0_1.view(-1, 16, * ir_deconv_0_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem3_D_IR, confidence_tem_ir3_D_IR, _, _, _ = self.tem_branch4_D( ir_deconv_0_1)
        # x_spat3_D_IR, conf_spat_ir3_D_IR, attn_maps_all_spat3_D_IR = self.spat_branch4_D( ir_deconv_0_1)
        # ir_deconv_0_1 = x_tem3_D_IR + x_spat3_D_IR +  ir_deconv_0_1 # (bs,16,256,14,14)
        #
        # ir_deconv_0_1 =  ir_deconv_0_1.view(-1, * ir_deconv_0_1.shape[-3:])
        # ir_deconv_1_1 = self.deconv_1_1(ir_deconv_0_1, ir_features3__)
        # ir_deconv_1_1 = ir_deconv_1_1.view(-1, 16, *ir_deconv_1_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem2_D_IR, confidence_tem_ir2_D_IR, _, _, _ = self.tem_branch3_D(ir_deconv_1_1)
        # x_spat2_D_IR, conf_spat_ir2_D_IR, attn_maps_all_spat2_D_IR = self.spat_branch3_D(ir_deconv_1_1)
        # ir_deconv_1_1 = x_tem2_D_IR + x_spat2_D_IR +  ir_deconv_1_1  # (bs,16,256,14,14)
        #
        # ir_deconv_1_1 =  ir_deconv_1_1.view(-1, * ir_deconv_1_1.shape[-3:])
        # ir_deconv_2_1 = self.deconv_2_1(ir_deconv_1_1, ir_features2__)
        # ir_deconv_2_1 = ir_deconv_2_1 .view(-1, 16, *ir_deconv_2_1 .shape[1:])  # (bs,16,512,7,7)
        # x_tem1_D_IR, confidence_tem_ir1_D_IR, _, _, _ = self.tem_branch2_D(ir_deconv_2_1 )
        # x_spat1_D_IR, conf_spat_ir1_D_IR, attn_maps_all_spat1_D_IR = self.spat_branch2_D(ir_deconv_2_1 )
        # ir_deconv_2_1 = x_tem1_D_IR + x_spat1_D_IR + ir_deconv_2_1  # (bs,16,256,14,14)
        #
        # ir_deconv_2_1 = ir_deconv_2_1.view(-1, *ir_deconv_2_1.shape[-3:])
        # ir_deconv_3_1 = self.deconv_3_1_(ir_deconv_2_1, x_ir1)
        # ir_output1 = self.out1(ir_deconv_3_1)
# -----------------------------------visible decoder---------------------------------------------------------------------------------
        ...
        # vis_deconv_0_1 = self.deconv_0_1(vis_features5__, vis_features4__)
        # vis_deconv_0_1 = vis_deconv_0_1.view(-1, 16, *vis_deconv_0_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem3_D_VIS, confidence_tem_vis3_D_VIS, _, _, _ = self.tem_branch4_D(vis_deconv_0_1)
        # x_spat3_D_VIS, conf_spat_vis3_D_VIS, attn_maps_all_spat3_D_VIS = self.spat_branch4_D(vis_deconv_0_1)
        # vis_deconv_0_1 = x_tem3_D_VIS + x_spat3_D_VIS + vis_deconv_0_1  # (bs,16,256,14,14)
        #
        # vis_deconv_0_1 = vis_deconv_0_1.view(-1, *vis_deconv_0_1.shape[-3:])
        # vis_deconv_1_1 = self.deconv_1_1(vis_deconv_0_1, vis_features3__)
        # vis_deconv_1_1 = vis_deconv_1_1.view(-1, 16, *vis_deconv_1_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem2_D_VIS, confidence_tem_vis2_D_VIS, _, _, _ = self.tem_branch3_D(vis_deconv_1_1)
        # x_spat2_D_VIS, conf_spat_ir2_D_VIS, attn_maps_all_spat2_D_VIS = self.spat_branch3_D(vis_deconv_1_1)
        # vis_deconv_1_1 = x_tem2_D_VIS + x_spat2_D_VIS + vis_deconv_1_1  # (bs,16,256,14,14)
        #
        # vis_deconv_1_1 = vis_deconv_1_1.view(-1, *vis_deconv_1_1.shape[-3:])
        # vis_deconv_2_1 = self.deconv_2_1(vis_deconv_1_1, vis_features2__)
        # vis_deconv_2_1 = vis_deconv_2_1.view(-1, 16, *vis_deconv_2_1.shape[1:])  # (bs,16,512,7,7)
        # x_tem1_D_VIS, confidence_tem_vis1_D_VIS, _, _, _ = self.tem_branch2_D(vis_deconv_2_1)
        # x_spat1_D_VIS, conf_spat_vis1_D_VIS, attn_maps_all_spat1_D_VIS = self.spat_branch2_D(vis_deconv_2_1)
        # vis_deconv_2_1 = x_tem1_D_VIS + x_spat1_D_VIS + vis_deconv_2_1  # (bs,16,256,14,14)
        #
        # vis_deconv_2_1 = vis_deconv_2_1.view(-1, *vis_deconv_2_1.shape[-3:])
        # vis_deconv_3_1 = self.deconv_3_1_(vis_deconv_2_1, y_vis1)
        # vis_output1 = self.out1(vis_deconv_3_1)
        ...
        return output1, ir_output1, vis_output1, lb_loss_4 , sparse_loss_4
...
if __name__ == "__main__":
   # 初始化模型
    ...
    model = ModifiedResNet18_fusion(32).to(device)
    #模拟输入 (bs=2, 16帧)
