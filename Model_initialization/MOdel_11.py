...
def load_model(model, model_path, opt, optimizer=None):
    ...
    # model_state_dict = model.dlaseg.state_dict()
    ...
    for k in model_state_dict:
        if not (k in state_dict):
            ...
            state_dict[k] = model_state_dict[k]
    # model.fuse_model.load_state_dict(torch.load('/home/zhuyabin/MOT/CenterTrack/models/IFCNN-MAX.pth'))
    # model.dlaseg.load_state_dict(state_dict, strict=False)
    model.load_state_dict(state_dict, strict=False)
    # pdb.set_trace()
    # pdb.set_trace()
    # head_dict2 = {k[len('head.'):]: v for k, v in pretrainmodel_single.items() if k[len('head.filter_predictor.transformer.'):] in state_dict}
    # head_filter_predictor_transformer_decoder_dict2 = {k[len('head.filter_predictor.transformer.'):]: v for k, v in pretrainmodel_single.items() if k[len('head.filter_predictor.transformer.'):] in head_filter_predictor_transformer_decoder_dict}
    # head.load_state_dict(head_dict2,strict=False)

    # base.base_layer.1.running_var
